<template>
  <div
    ref="containerRef"
    class="card-swap-container absolute bottom-0 right-0 rounded-2xl
           transform translate-x-[-3rem] translate-y-[0rem] 
  ...
  >
    <div
      ...
      :key="index"
      ref="cardRefs"
      class="card-swap-card absolute top-1/2 left-1/2 rounded-xl border border-white bg-black text-white [transform-style:preserve-3d] [will-change:transform] [backface-visibility:hidden]"
      :style="{
        width: typeof width === 'number' ? `${width}px` : width,
        height: typeof height === 'number' ? `${height}px` : height
      }"
