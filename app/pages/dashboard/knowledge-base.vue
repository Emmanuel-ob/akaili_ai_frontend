  ...
  >
    ...
    >
      ...
      >
        <div class="space-y-1">
          <h1
            class="text-[#9E4CFF] text-2xl lg:text-3xl font-bold font-heading leading-tight"
          >
            Knowledge Base
          </h1>
          <p
            class="text-sm lg:text-base text-gray-500 dark:text-gray-400 font-sans"
          >
        ...
        <div class="w-full sm:w-auto">
          ...
          <select
            ...
            class="w-full sm:w-auto px-4 py-2 border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-colors cursor-pointer font-sans text-sm"
            @change="onChatbotChange"
          >
            <option value="" selected hidden disabled>Select Chatbot</option>
            <option
              v-for="bot in chatbotStore.chatbots"
              :key="bot.id"
              :value="bot.id"
            >
              {{ bot.name }}
    ...
    <main class="px-4 sm:px-6 py-6 sm:py-8">
      ...
      >
        <div
          class="bg-gray-50 dark:bg-slate-800 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4"
        >
          <svg
        ...
        <p
          class="text-lg font-semibold text-gray-900 dark:text-white mb-2 font-sans"
        >
          Select a Chatbot
        </p>
        <p class="text-sm text-gray-500 dark:text-gray-400 font-sans">
          Choose a chatbot above to manage its knowledge base
        </p>
      </div>
      ...
      <div v-else>
        ...
        <div
          class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 transition-all duration-700 delay-100"
          :class="
            isPageLoaded
        ...
        >
          <StatCard
            title="Total Sources"
            :value="faqStore.faqSources.length"
            icon="folder"
            color="blue"
          />
          <StatCard
        ...
        <div
          class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8 transition-all duration-700 delay-200"
          :class="
            isPageLoaded
              ? 'opacity-100 translate-y-0'
              : 'opacity-0 translate-y-4'
        ...
        >
          <div
            :class="[
              'cursor-pointer rounded-lg border p-6 transition-all duration-200 flex items-center justify-between group',
              activeTab === 'upload'
          ...
          >
            <div class="space-y-1">
              <h3
                class="text-lg font-semibold text-gray-900 dark:text-white group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors font-subheading"
              >
                Upload Documents
              </h3>
              <p class="text-sm text-gray-600 dark:text-gray-400 font-sans">
                PDF, Word, Excel, CSV, TXT, JSON
              </p>
            </div>
            <div
              ...
              :class="
                activeTab === 'upload'
                  ? 'bg-white dark:bg-purple-900/30'
        ...
        <div
          class="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-800 p-4 sm:p-6 transition-all duration-700 delay-500"
          :class="
            isPageLoaded
              ? 'opacity-100 translate-y-0'
              : 'opacity-0 translate-y-4'
        ...
        >
          ...
          >
            <div class="flex items-center gap-3">
              <h3
                class="text-lg font-semibold text-gray-900 dark:text-white font-subheading"
              >
                FAQ Sources
              </h3>
              <span
                class="px-2.5 py-0.5 rounded-full bg-gray-100 dark:bg-slate-800 text-xs font-medium text-gray-600 dark:text-gray-400 font-sans"
              >
            ...
            <button
              :disabled="isRefreshing || faqStore.loading"
              class="p-2 text-gray-600 dark:text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg disabled:opacity-50 transition-all font-sans text-sm"
              title="Refresh List"
              @click="triggerReloadAnimation"
            >
              <svg
    ...
    >
      <template #body>
        <div
          ref="deleteModalContainer"
          class="text-center py-2"
          tabindex="0"
          @keydown.enter.prevent="confirmDelete"
        >
          <div
            class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-red-50 dark:bg-red-500/10 mb-6 transition-colors duration-300"
          >
            <svg
          ...
          <p
            class="text-sm text-gray-500 dark:text-slate-400 max-w-sm mx-auto leading-relaxed font-sans"
          >
            Are you sure you want to delete
            <strong class="text-gray-900 dark:text-white font-sans">{{
              faqToDelete?.source_name
            }}</strong
            >? This action cannot be undone.
          </p>
          <p class="text-xs text-gray-400 dark:text-slate-500 mt-4 font-sans">
            Press
            <kbd
              class="px-2 py-1 bg-gray-100 dark:bg-slate-800 rounded border border-gray-200 dark:border-slate-700 font-sans text-[10px]"
              >Enter</kbd
            >
            to confirm
          </p>
        ...
        <div class="mt-8 flex justify-end gap-3">
          <button
            class="px-5 py-2.5 bg-white dark:bg-slate-800 text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-slate-700 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 focus:outline-none transition-colors font-medium font-sans text-sm"
          ...
          <button
            ...
            class="px-5 py-2.5 bg-red-600 hover:bg-red-700 dark:bg-red-600 dark:hover:bg-red-700 text-white rounded-lg shadow-lg shadow-red-500/20 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 dark:focus:ring-offset-slate-900 transition-colors font-medium font-sans text-sm"
            @click="confirmDelete"
          >
            Delete Source
          </button>
...
const activeSources = computed(
  () => faqStore.faqSources.filter((faq) => faq.is_active).length,
);
const totalItems = computed(() =>
  faqStore.faqSources.reduce((sum, faq) => sum + (faq.total_items || 0), 0),
);
const embeddedItems = computed(() =>
  faqStore.faqSources.reduce((sum, faq) => sum + (faq.embedded_items || 0), 0),
);
  ...
  toast.success(
    {
      ...
      props: {
        ...
        actionLabel: "Review",
        onAction: () => {
          const faq = faqStore.faqSources.find(
            (f) => f.id === event.faq_source.id,
          );
...
const handleReprocess = async (faqSource) => {
  if (
    confirm(
      `This will recreate the embeddings for "${faqSource.source_name}". Continue?`,
    )
  ) {
