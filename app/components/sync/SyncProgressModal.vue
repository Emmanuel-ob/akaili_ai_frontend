<template>
  <BaseModal :show="show" :title="modalTitle" @close="handleClose">
    <div class="space-y-6">
      ...
      >
        <h4
          class="text-sm font-medium text-red-800 mb-2 font-subheading leading-tight"
        >
          Error Details
        </h4>
        <p class="text-sm text-red-700 font-sans">{{ errorMessage }}</p>
      </div>

      <!-- Failed Items -->
      <div v-if="failedItems && failedItems.length > 0" class="space-y-2">
        <button
          class="flex items-center justify-between w-full text-left text-sm font-medium text-gray-900 font-sans"
          @click="showFailedItems = !showFailedItems"
        >
          <span>Failed Items ({{ failedItems.length }})</span>
          <svg
            class="w-4 h-4 transform transition-transform"
            :class="{ 'rotate-180': showFailedItems }"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <div
          v-if="showFailedItems"
          class="bg-red-50 rounded-lg p-3 max-h-40 overflow-y-auto font-sans"
        >
          <div
            v-for="(item, index) in failedItems.slice(0, 10)"
            :key="index"
      ...
      <!-- Action Buttons -->
      <div class="flex justify-between items-center pt-4 font-sans">
        <div>
          <button
            v-if="status === 'running'"
        ...
        <div class="flex space-x-3">
          ...
          <button
            v-if="status === 'failed'"
            class="px-4 py-2 text-sm text-white bg-[#9E4CFF] hover:bg-purple-700 rounded-md"
            @click="retrySync"
          >
            Retry Sync
          </button>
  ...
  progressInterval = setInterval(async () => {
    try {
      const response = await $fetch(
        `${config.public.apiBase}/api/sync/${props.connectionId}/status`,
        {
          headers: {
...
const cancelSync = async () => {
  try {
    await $fetch(
      `${config.public.apiBase}/api/sync/${props.connectionId}/cancel`,
      {
        method: "POST",
        headers: {
