<template>
  ...
  >
    <div
      class="bg-white rounded-lg w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col"
    >
      <!-- Header -->
      <div
      ...
      >
        <div class="flex items-center gap-4">
          <h2 class="text-2xl font-bold text-gray-900 font-subheading">
            {{ isEditing ? "Edit Campaign" : "Create Campaign" }}
          </h2>
          <span
            v-if="autoSaveStatus"
            class="text-sm text-gray-600 px-3 py-1 bg-blue-50 rounded-lg"
          >
            {{ autoSaveStatus }}
          </span>
        </div>
          ...
          <div class="flex items-center justify-between mb-4">
            ...
            >
              ...
              <div
                ...
                class="w-full h-0.5 bg-gray-200 mx-2"
              >
                <div
    ...
    >
      ...
      <div class="flex-1 overflow-y-auto p-6">
        ...
        <div v-show="currentStep === 1" class="space-y-8 max-w-4xl mx-auto">
              ...
              <div class="space-y-2">
                <label
              class="block text-sm font-semibold text-gray-700 dark:text-slate-300 font-subheading"
              >Campaign Name *</label
                >
            <input
          ...
          <div>
            ...
            <input
              ...
              type="text"
              placeholder="e.g., 🔥 Big Summer Sale - 50% Off!"
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                type="text"
                placeholder="e.g., John from Xeli AI"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
                required
              />
            </div>
            <div>
              ...
              <input
                ...
                type="email"
                placeholder="e.g., john@xeliai.com"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
                required
              />
            </div>
            ...
            <input
              ...
              type="email"
              placeholder="support@xeliai.com"
                  class="w-full px-4 py-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-purple-500/20 focus:border-purple-600 outline-none transition-all"
            />
                        </div>
          </div>
        ...
        <div v-show="currentStep === 2" class="space-y-8 max-w-4xl mx-auto">
          <div class="space-y-2">
            ...
            >
              ...
              <option
                ...
                :key="list.id"
                :value="list.id" class="dark:bg-slate-900"
              >
          ...
          <div v-if="formData.list_id && availableTags.length > 0">
            <label
              class="block text-sm font-medium text-gray-700 mb-2 font-sans"
            >
              Filter by Tags (optional)
            </label>
            <div
              class="border border-gray-300 rounded-lg p-4 space-y-2 max-h-48 overflow-y-auto"
            >
              <label
        ...
        <div v-show="currentStep === 3" class="space-y-6">
          ...
          <div class="border border-gray-200 dark:border-slate-800 sm:rounded-2xl overflow-hidden -mx-5 sm:mx-0">
            <div id="unlayer-editor" style="height: 600px" />
          </div>
        ...
        <div v-show="currentStep === 4" class="space-y-6">
          <div>
            <label
              class="block text-sm font-medium text-gray-700 mb-4 font-subheading"
              >When to send?</label
            >
            <div class="space-y-3">
              <label
          ...
          <div v-if="sendOption === 'schedule'" class="p-6 bg-gray-50 dark:bg-slate-800/50 rounded-2xl border border-gray-100 dark:border-slate-800">
            <label
              class="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-2 font-subheading"
          ...
          <div class="p-6 sm:p-8 bg-slate-900 dark:bg-black rounded-3xl text-white shadow-xl">
            <h3
              class="text-xl font-bold mb-6 border-b border-white/10 pb-4 font-subheading"
            >
              Campaign Summary
            </h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div class="space-y-1">
                ...
                <p class="font-medium"
                  >{{ selectedContactsCount }} contacts</p
                >
              </div>
              <div class="space-y-1">
              ...
              <div class="space-y-1">
                ...
                <p class="font-medium text-purple-400">
                  {{
                    sendOption === "now"
                      ? "Instant Blast"
                      : formatScheduledDate(formData.send_date)
                  }}
                </p>
              </div>
            </div>
...
const autoSave = async () => {
  ...
  try {
    ...
    campaignId.value = await emailStore.autoSaveCampaign(
      formData.value,
      campaignId.value,
    );
...
const validateCurrentStep = () => {
  ...
  if (currentStep.value === 1) {
    if (!formData.value.name)
      validationErrors.value.push("Campaign name is required");
    if (!formData.value.subject)
      validationErrors.value.push("Subject line is required");
    if (!formData.value.from_name)
      validationErrors.value.push("From name is required");
    if (!formData.value.from_email)
      validationErrors.value.push("From email is required");
  } else if (currentStep.value === 2) {
    if (!formData.value.list_id)
      validationErrors.value.push("Email list is required");
    if (selectedContactsCount.value === 0)
      validationErrors.value.push("List must have at least one contact");
  }
  if (currentStep.value === 1) {
    if (!formData.value.name)
      validationErrors.value.push("Campaign name is required");
...
const selectTemplate = async (template) => {
  ...
  if (template.unlayer_design) {
    const design =
      typeof template.unlayer_design === "string"
  ...
  try {
    unlayer.exportHtml(async (data) => {
      ...
      if (sendOption.value === "now") {
        ...
        // Schedule
        const campaign = await emailStore.createCampaign(formData.value);
        await emailStore.scheduleCampaign(
          campaign.id,
          formData.value.send_date,
        );
...
onMounted(async () => {
  ...
  if (isEditing.value) {
    ...
    if (props.campaign.unlayer_design) {
      const design =
        typeof props.campaign.unlayer_design === "string"
