    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">View all your invoices and payment transactions
                </p>
            </div>
        ...
        <!-- Loading State -->
        <div v-if="loading && invoices.length === 0" class="animate-pulse space-y-4">
        ...
        <!-- Empty State -->
        <div v-else-if="!loading && invoices.length === 0"
            ...
            <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-2">No Invoices Yet</h3>
            ...
            <div v-for="invoice in invoices" :key="invoice.id"
                ...
                <div class="flex items-start justify-between">
                            ...
                            <span :class="[
                                ...
                                invoice.status === 'paid' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' :
                                    invoice.status === 'pending' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300' :
                                        invoice.status === 'failed' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300' :
                                            'bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-300'
                            ]">
                                {{ invoice.status === 'paid' ? 'Paid' : invoice.status === 'pending' ? 'Pending' :
                                    invoice.status === 'failed' ? 'Failed' : invoice.status }}
                            </span>
                        </div>

                        <div class="space-y-1 text-sm text-gray-600 dark:text-gray-400">
                            <p>Amount: <span class="font-semibold text-gray-900 dark:text-white">{{
                                    invoice.formatted_amount }}</span></p>
                            <p>Date: {{ formatDate(invoice.created_at) }}</p>
                            <p v-if="invoice.paid_at">Paid on: {{ formatDate(invoice.paid_at) }}</p>
                            <p v-if="invoice.payment_provider" class="flex items-center gap-1">
                                Provider:
                    ...
                    <div class="flex gap-2">
                        ...
                        <button v-if="invoice.status === 'paid'" @click="downloadInvoice(invoice.id)"
                            class="px-3 py-1.5 text-sm border border-gray-300 dark:border-slate-700 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700 transition">
                            Download
                        </button>
                    </div>
        ...
        <InvoiceDetailModal v-if="selectedInvoice" :invoice="selectedInvoice" @close="selectedInvoice = null" />
    </div>
</template>
...
const fetchInvoices = async (page = 1) => {
    ...
    try {
        const response = await $fetch(`${config.public.apiBase}/api/billing/invoices?page=${page}`, {
...
const viewInvoice = async (invoiceId) => {
    try {
        const response = await $fetch(`${config.public.apiBase}/api/billing/invoices/${invoiceId}`, {
            headers: { Authorization: `Bearer ${authStore.token}` }
        })
...
const downloadInvoice = async (invoiceId) => {
    try {
        ...
        const response = await $fetch(`${config.public.apiBase}/api/billing/invoices/${invoiceId}/download`, {
            headers: { Authorization: `Bearer ${authStore.token}` }
        })
