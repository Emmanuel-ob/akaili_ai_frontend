<template>
  <div class="space-y-4">
    <!-- Empty State -->
    <div
      v-if="!faqSources.length && !loading"
      class="text-center py-12 bg-gray-50 dark:bg-slate-800/50 rounded-lg border-2 border-dashed border-gray-300 dark:border-slate-700"
    >
      <svg
      ...
      <p
        class="text-lg font-medium text-gray-700 dark:text-gray-200 mb-2 font-sans"
      >
        No FAQ sources yet
      </p>
      <p class="text-sm text-gray-500 dark:text-gray-400 font-sans">
        Upload documents or create manual Q&A pairs to get started
      </p>
    </div>

    <!-- FAQ Source Cards -->
    <div
      v-for="faq in faqSources"
      :key="faq.id"
      class="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-800 p-4 sm:p-6 hover:shadow-md transition-shadow"
    >
      <!-- Header Section -->
      <div
        class="flex flex-col sm:flex-row sm:items-start justify-between mb-4 gap-4 sm:gap-0"
      >
        <div class="flex-1 min-w-0">
          <!-- min-w-0 forces truncation inside flex items -->
    ...
    <div class="space-y-4">
        ...
        <div v-for="faq in faqSources" :key="faq.id"
            ...
            <span
              :class="[
                'px-2 py-1 text-xs font-medium rounded-full font-sans',
            ...
            <span
              v-if="faq.is_active"
              class="px-2 py-0.5 text-xs font-medium rounded-full bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 shrink-0 font-sans"
            >
              ACTIVE
            </span>
          ...
          <!-- Meta Data -->
          <div
            class="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-gray-500 dark:text-gray-400 font-sans"
          >
            ...
            <span
              v-if="faq.file_name"
              class="truncate max-w-[150px] sm:max-w-[200px] font-subheading"
              :title="faq.file_name"
            >
              {{ faq.file_name }}
            </span>
            <span class="whitespace-nowrap font-sans"
              >{{ faq.total_items || 0 }} items</span
            >
            <span v-if="faq.embedded_items" class="whitespace-nowrap font-sans"
              >{{ faq.embedded_items }} embedded</span
            >
          </div>
        </div>
        ...
        <div class="flex sm:block shrink-0">
          <span
            class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 font-sans"
          >
      ...
      <!-- Progress Bar -->
      <div
        v-if="faq.status === 'processing' || faq.status === 'pending'"
        class="mb-4"
      >
        <div
          class="flex items-center justify-between text-sm text-gray-600 dark:text-gray-400 mb-1 font-sans"
        >
          <span>{{
            faq.status === "pending" ? "Queued..." : "Processing..."
          }}</span>
        ...
        <div class="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-2">
          ...
          >
            <div
              v-if="faq.status === 'processing'"
              class="h-full w-full bg-linear-to-r from-transparent via-white/30 to-transparent opacity-30 animate-shimmer"
            />
          </div>
        </div>
      </div>
      ...
      <div
        v-if="faq.status === 'failed' && faq.error_message"
        ...
        <div class="flex items-start justify-between">
          <div class="flex-1">
            <div class="flex items-center space-x-2 mb-2">
              ...
              <p class="text-sm font-medium text-red-800 font-sans">
            ...
            <p class="text-xs text-red-600 ml-7 font-sans">
              {{ faq.error_message }}
            </p>
          </div>
          <button
            :disabled="isProcessing(faq.id)"
            :class="[
              'ml-3 px-3 py-1.5 text-xs font-medium rounded transition-colors font-sans',
              isProcessing(faq.id)
                ? 'bg-red-200 text-red-400 cursor-not-allowed'
                : 'bg-red-600 text-white hover:bg-red-700',
            ]"
            @click="handleRetryAction(faq)"
          >
            <span v-if="isProcessing(faq.id)" class="inline-flex items-center">
              <svg
                class="animate-spin h-3 w-3 mr-1 font-sans"
                fill="none"
                viewBox="0 0 24 24"
              >
                <circle
            ...
            <span v-else>{{
              getRetryButtonLabel(faq.last_failed_action)
            }}</span>
          </button>
        </div>
      </div>
      ...
      >
        <div class="flex items-center space-x-2">
          <!-- Priority Slider -->
          <div class="flex items-center space-x-2">
            <label class="text-sm text-gray-600 font-subheading"
              >Priority:</label
            >
            <input
              :value="faq.priority"
              ...
              min="1"
              max="10"
              class="w-24 font-sans text-sm"
              :disabled="isProcessing(faq.id)"
              @change="updatePriority(faq.id, $event.target.value)"
            />
            <span class="text-sm font-medium text-gray-900 w-6 font-sans">{{
              faq.priority
            }}</span>
          </div>
        </div>
        ...
        <div class="flex items-center space-x-2">
          ...
          <button
            v-if="faq.source_type === 'manual_qa' && !isProcessing(faq.id)"
            class="px-3 py-1.5 text-sm text-purple-700 border border-purple-300 rounded-md hover:bg-purple-50 disabled:opacity-50 disabled:cursor-not-allowed font-sans"
            @click="$emit('edit', faq)"
          >
            Edit Q&A
          </button>
          ...
          <!-- Preview Button -->
          <button
            v-if="faq.status === 'completed' && !isProcessing(faq.id)"
            class="px-3 py-1.5 text-sm text-gray-700 border border-gray-300 rounded-md hover:bg-gray-50 font-sans"
            @click="$emit('preview', faq)"
          >
            Preview
          </button>
          ...
          <button
            v-if="faq.status === 'completed' && faq.embedded_items === 0"
            :disabled="isProcessing(faq.id)"
            :class="[
              'px-3 py-1.5 text-sm text-white rounded-md transition-colors font-sans',
              isProcessing(faq.id)
                ? 'bg-purple-400 cursor-not-allowed'
                : 'bg-purple-600 hover:bg-purple-700',
            ]"
            @click="$emit('embed', faq)"
          >
            <span v-if="isProcessing(faq.id)" class="inline-flex items-center">
              <svg
                class="animate-spin h-4 w-4 mr-2 font-sans"
                fill="none"
                viewBox="0 0 24 24"
              >
                <circle
          ...
          <button
            v-if="faq.embedded_items > 0 && faq.status !== 'processing'"
            :disabled="isProcessing(faq.id)"
            :class="[
              'px-3 py-1.5 text-sm rounded-md border transition-colors font-sans',
              isProcessing(faq.id)
          ...
          <!-- Toggle Active -->
          <button
            v-if="faq.embedded_items > 0 && !isProcessing(faq.id)"
            :class="[
          ...
          <button
            :disabled="isProcessing(faq.id)"
            :class="[
              'p-2 rounded-md transition-colors font-sans',
              isProcessing(faq.id)
