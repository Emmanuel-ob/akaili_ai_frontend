...
<template>
  <NuxtLink
    :to="to"
    class="relative group bg-gray-50 p-6 rounded-lg hover:bg-gray-100 transition-colors"
  >
    <div class="flex items-center">
      <div class="w-10 h-10 rounded-lg flex items-center justify-center mr-4" :class="iconBgClass">
        <svg class="w-6 h-6" :class="iconTextClass" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="iconPath"/>
        </svg>
      </div>
      <div>
        <h4 class="text-sm font-medium text-gray-900" :class="titleHoverClass">
          {{ title }}
        </h4>
