    ...
    <div>
        ...
        <div class="flex flex-col xl:flex-row items-start xl:items-center justify-between mb-6 gap-4">
            ...
            <div class="flex flex-wrap items-center gap-2 w-full xl:w-auto">
                <button @click="showHelp = !showHelp"
                    class="px-4 py-2 text-blue-600 dark:text-blue-400 border border-blue-300 dark:border-blue-700 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">
                ...
                <button @click="exportContacts" class="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition-colors">
                    Export
                </button>
                <button @click="openAddModal" class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors">
                    Add Contact
                </button>
            </div>
        </div>
        ...
        <transition name="slide-down">
            <div v-if="showHelp" class="mb-6">
                <div class="bg-gradient-to-br from-blue-50 to-purple-50 dark:from-slate-800 dark:to-slate-800 rounded-lg border border-blue-200 dark:border-slate-700 p-6">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-3">Custom Fields Guide</h3>

                            <div class="grid md:grid-cols-2 gap-4">
                                <!-- Manual Entry -->
                                <div class="bg-white dark:bg-slate-900 rounded-lg p-4 shadow-sm border border-gray-100 dark:border-slate-700">
                                ...
                                <div class="bg-white dark:bg-slate-900 rounded-lg p-4 shadow-sm border border-gray-100 dark:border-slate-700">
                                    ...
                                    <div class="bg-gray-50 dark:bg-slate-800 rounded p-2 font-mono text-xs overflow-x-auto border border-gray-200 dark:border-slate-700">
                                        <div class="text-gray-500 dark:text-gray-400">email, name, Company, Title</div>
                                        <div class="text-gray-700 dark:text-gray-300">john@ex.com, John, Acme, CEO</div>
                                    </div>
                                ...
                                <div class="bg-white dark:bg-slate-900 rounded-lg p-4 shadow-sm border border-gray-100 dark:border-slate-700">
                                    ...
                                    <div class="bg-purple-50 dark:bg-slate-800 rounded p-2 text-sm border border-purple-100 dark:border-slate-700">
                                        <code class="text-purple-700 dark:text-purple-400">Hi {{ first_name }}</code><br>
                                        <code class="text-purple-700 dark:text-purple-400">from {{ Company }}</code>
                                    </div>
                                </div>
        ...
        <div v-else-if="emailStore.contacts.length === 0" class="text-center py-12">
            <div class="bg-gray-100 dark:bg-slate-800 rounded-full w-20 h-20 flex items-center justify-center mx-auto mb-4">
                <svg class="w-10 h-10 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                </svg>
            </div>
            <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-2">No contacts yet</h3>
            <p class="text-gray-600 dark:text-gray-400 mb-4">Add your first contact or import from CSV</p>
            <div class="flex items-center justify-center gap-3">
                <button @click="showImportModal = true"
                    class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors">
                    Import Contacts
                </button>
                <button @click="openAddModal" class="px-6 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors">
                    Add Contact
                </button>
            </div>
        </div>
        ...
        <!-- Contacts Table -->
        <div v-else class="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-800 overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-slate-800">
                    <tr v-for="contact in emailStore.contacts" :key="contact.id" class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                        <td class="px-6 py-4">
                            <div>
                                <p class="font-medium text-gray-900 dark:text-white">{{ contact.first_name }} {{ contact.last_name }}
                                </p>
                        ...
                        <td class="px-6 py-4">
                            ...
                            <span v-else class="text-sm text-gray-400 dark:text-gray-500">No custom fields</span>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <span
                                :class="['px-2 py-1 text-xs font-medium rounded-full',contact.status === 'active' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' : contact.status === 'unsubscribed' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300']">
                                {{ contact.status }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <button @click="editContact(contact)" class="text-purple-600 dark:text-purple-400 hover:text-purple-800 dark:hover:text-purple-300 transition-colors">
                                    Edit
                                </button>
                                <button @click="deleteContact(contact)" class="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 transition-colors">
                                    Delete
                                </button>
                            </div>
                        </td>
        ...
        <div v-if="emailStore.contacts.length > 0" class="mt-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <button @click="changePage(emailStore.contactsPagination.current_page - 1)"
                    ...
                    class="px-3 py-1 border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-700 dark:text-gray-300 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
                    Previous
                </button>
