...
const handleSocialAuth = (provider) => {
  // UPDATED: Store plan in sessionStorage before opening social auth
  const pendingPlan = route.query.plan
  if (pendingPlan) {
    sessionStorage.setItem('pendingPlan', pendingPlan)
  }

  emit('social-auth', provider)
}
</script>
...
<template>
  <div class="space-y-3">
    <button
      type="button"
      class="w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-300 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors"
