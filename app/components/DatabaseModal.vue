  ...
  >
    <form class="space-y-6" @submit.prevent="handleSubmit">
        ...
        <div class="space-y-4">
            ...
            >
              ...
              <p>
                ...
                <code class="bg-blue-100 px-1 rounded"
                  >mysql://user:pass@host:3306/dbname</code
                >
              </p>
              <p>
                PostgreSQL:
                <code class="bg-blue-100 px-1 rounded"
                  >postgresql://user:pass@host:5432/dbname</code
                >
              </p>
              <p>
                MongoDB:
                <code class="bg-blue-100 px-1 rounded"
                  >mongodb+srv://user:pass@host/dbname</code
                >
              </p>
            </div>
          </div>
          ...
          <div v-if="canLoadDatabases && !showDatabaseSelect">
            <button
              ...
              class="w-full px-4 py-2 text-sm text-purple-600 border border-purple-200 rounded-md hover:bg-purple-50 disabled:opacity-50 font-sans"
          ...
          <div
            ...
            class="p-3 bg-yellow-50 border border-yellow-200 rounded-md text-sm text-yellow-800 font-sans"
          >
          ...
          <div
            v-if="error"
            class="p-3 bg-red-50 border border-red-200 rounded-md text-sm text-red-600 font-sans"
          >
            {{ error }}
          </div>
          ...
          <div
            ...
            class="p-3 bg-blue-50 border border-blue-200 rounded-md text-sm text-blue-600 font-sans"
          >
          ...
          <div v-if="showDatabaseSelect" class="space-y-4">
            ...
            <div
              ...
              class="mt-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800 font-sans"
            >
              <strong>⚠️ Warning:</strong>
              {{ getDatabaseWarning(form.database) }}
            </div>

            <!-- Schemas for PostgreSQL -->
            <div
              v-if="form.type === 'postgresql' && availableSchemas.length > 0"
              class="space-y-2"
            >
              <label class="block text-sm font-medium text-gray-700 font-sans"
                >Select Schemas <span class="text-red-500">*</span></label
              >
              <div
                class="space-y-2 max-h-32 overflow-y-auto border border-gray-200 rounded p-2"
              >
                <label
                  v-for="schema in availableSchemas"
                  :key="schema"
                  class="flex items-center p-1 hover:bg-gray-50 rounded cursor-pointer font-sans"
                >
                  <input
              ...
              <p class="text-xs text-gray-500 font-sans">
                Selected schemas will be used for table filtering
              </p>

              <div
                v-if="selectedSchemas.length === 0"
                class="p-2 bg-yellow-50 border border-yellow-200 rounded-md text-sm text-yellow-800 font-sans"
              >
            ...
            <button
              type="button"
              class="text-sm text-gray-600 hover:text-gray-800 underline font-sans"
      ...
      <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200">
        <button
          type="button"
          class="px-4 py-2 text-sm text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-50 font-sans"
          @click="$emit('close')"
        >
          Cancel
        </button>
        <button
          ...
          class="px-4 py-2 text-sm text-white bg-[#9E4CFF] hover:bg-purple-700 rounded-lg disabled:opacity-50 font-sans"
        >
...
const getDatabaseWarning = (database) => {
  const warningDb = databasesWithWarnings.value.find(
    (db) => db.name === database,
  );
