...
<template>
    ...
    >
        ...
        <!-- Chat Header -->
        <div class="bg-[#9E4CFF] dark:bg-[#7C3AED] text-white px-6 py-4 flex justify-between items-center shrink-0 select-none cursor-move" @mousedown="startDrag">
            <div>
        ...
        <div v-if="handoverStatus === 'requested'" class="bg-yellow-50 dark:bg-yellow-900/30 border-b border-yellow-200 dark:border-yellow-800/50 p-3 shrink-0">
            <div class="flex items-center text-yellow-800 dark:text-yellow-200 text-sm">
                <div class="animate-spin mr-2 w-4 h-4 border-2 border-yellow-600 dark:border-yellow-400 border-t-transparent rounded-full"></div>
                <span>Finding an available agent...</span>
            </div>
        </div>

        <div v-if="handoverStatus === 'rejected'" class="bg-red-50 dark:bg-red-900/30 border-b border-red-200 dark:border-red-800/50 p-3 shrink-0">
            <p class="text-red-800 dark:text-red-200 text-sm">
                ❌ All agents are busy. Please continue with the AI.
            </p>
        </div>

        <div v-if="handoverStatus === 'active'" class="bg-green-50 dark:bg-green-900/30 border-b border-green-200 dark:border-green-800/50 p-3 shrink-0">
        ...
        <div ref="messagesContainer" class="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 dark:bg-slate-950">
            ...
            <div v-for="(message, index) in messages" :key="index" :class="[
                ...
                <div :class="[
                    ...
                    <div v-if="message.role !== 'user'" class="prose prose-sm dark:prose-invert max-w-none break-words leading-relaxed" v-html="formatMessage(message.message)"></div>
                    <p v-else class="whitespace-pre-wrap leading-relaxed">{{ message.message }}</p>

                    <div v-if="message.role === 'assistant' && message.sources && message.sources.length > 0" class="mt-3 pt-2 border-t border-gray-100 dark:border-slate-700/50">
        ...
        <div class="p-4 border-t border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0">
            <div v-if="!handoverStatus && !isHandoverRequested" class="mb-3">
                <button @click="requestHumanAgent" class="w-full text-xs sm:text-sm text-[#9E4CFF] dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 font-medium py-2 border border-[#9E4CFF] dark:border-purple-500/50 rounded-lg hover:bg-purple-50 dark:hover:bg-purple-900/10 transition-colors">
                    💬 Speak with a Human Agent
                </button>
            </div>

            <div class="flex space-x-2">
                <input v-model="currentMessage" @keypress.enter="sendMessage" :disabled="isTyping" type="text" placeholder="Type your message..." class="flex-1 px-4 py-3 border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 disabled:bg-gray-100 dark:disabled:bg-slate-800/50 transition-colors placeholder-gray-400 dark:placeholder-gray-500" />
                <button @click="sendMessage" :disabled="!currentMessage.trim() || isTyping" class="px-5 py-3 bg-[#9E4CFF] hover:bg-purple-700 text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 disabled:opacity-50 transition-all shadow-md">
                    Send
                </button>
            </div>
        </div>
        ...
        <!-- Error Toast -->
        <div v-if="error" class="absolute bottom-20 left-4 right-4 bg-red-50 dark:bg-red-900/90 border border-red-200 dark:border-red-800 rounded-lg p-3 text-sm text-red-700 dark:text-red-200 text-center shadow-lg">
            {{ error }}
        </div>
        ...
        <!-- Right Handle -->
        <div class="absolute right-0 top-0 bottom-4 w-3 cursor-ew-resize hover:bg-purple-500/10 z-50 transition-colors" @mousedown.prevent="startResize('right', $event)"></div>

        <!-- Left Handle -->
        <div class="absolute left-0 top-0 bottom-4 w-3 cursor-ew-resize hover:bg-purple-500/10 z-50 transition-colors" @mousedown.prevent="startResize('left', $event)"></div>

        <!-- Bottom Handle -->
        <div class="absolute left-4 bottom-0 right-4 h-3 cursor-ns-resize hover:bg-purple-500/10 z-50 transition-colors" @mousedown.prevent="startResize('bottom', $event)"></div>

        <!-- Bottom-Right Corner -->
        <div class="absolute bottom-0 right-0 w-6 h-6 cursor-nwse-resize flex items-end justify-end p-1 z-50 hover:bg-purple-500/20 rounded-tl-lg" @mousedown.prevent="startResize('bottom-right', $event)">
            <div class="w-1.5 h-1.5 bg-gray-400 dark:bg-gray-600 rounded-full mb-1 mr-1"></div>
        </div>

        <!-- Bottom-Left Corner -->
        <div class="absolute bottom-0 left-0 w-6 h-6 cursor-nesw-resize flex items-end justify-start p-1 z-50 hover:bg-purple-500/20 rounded-tr-lg" @mousedown.prevent="startResize('bottom-left', $event)">
            <div class="w-1.5 h-1.5 bg-gray-400 dark:bg-gray-600 rounded-full mb-1 ml-1"></div>
        </div>
...
const resize = (e) => {
    ...
    // Bottom Resizing
    if (['bottom', 'bottom-right', 'bottom-left'].includes(resizeDirection.value)) {
...
const setupConversationListener = () => {
    ...
    conversationChannel.listen('.agent.message', (e) => { messages.value.push({ role: 'agent', message: e.message.message, timestamp: e.message.timestamp, metadata: { agent_name: e.agent.name } }); scrollToBottom() })
}

const sendMessage = async () => {
    if (!currentMessage.value.trim() || isTyping.value) return
    const msg = currentMessage.value.trim(); currentMessage.value = ''; error.value = ''
    messages.value.push({ role: 'user', message: msg, timestamp: new Date().toISOString() }); scrollToBottom(); isTyping.value = true
    try {
        const config = useRuntimeConfig(); const auth = useAuthStore()
        if (isHandoverActive.value && auth.token) {
            await $fetch(`${config.public.apiBase}/api/agent-handover/message`, { method: 'POST', headers: { Authorization: `Bearer ${auth.token}` }, body: { conversation_id: conversationId.value, message: msg } })
            isTyping.value = false; return
        }
        const res = await $fetch(`${config.public.apiBase}/api/chat`, { method: 'POST', headers: { Authorization: `Bearer ${auth.token}` }, body: { message: msg, chatbot_id: props.chatbotId, session_id: sessionId.value } })
        if (res.success) {
            if (res.conversation_id) conversationId.value = res.conversation_id
            if (!isHandoverActive.value) messages.value.push({ role: 'assistant', message: res.data.response.text, timestamp: new Date().toISOString(), sources: res.data.response.sources || [] })
        }
    } catch (e) { error.value = 'Failed to send'; isTyping.value = false }
    finally { isTyping.value = false; scrollToBottom() }
}

const resetChat = () => {
    if (conversationChannel) { $echo.leave(`conversation.${conversationId.value}`); conversationChannel = null }
    messages.value = []; sessionId.value = generateSessionId(); conversationId.value = null; error.value = ''; handoverStatus.value = null; isHandoverRequested.value = false
}

onUnmounted(() => { if (conversationChannel) $echo.leave(`conversation.${conversationId.value}`); window.removeEventListener('mouseup', stopResize) })
onMounted(() => { sessionId.value = generateSessionId() })
</script>

<style scoped>
:deep(.prose ul), :deep(.prose ol) { margin: 0.5rem 0; }
:deep(.prose li) { margin: 0.25rem 0; }
:deep(.prose strong) { font-weight: 700; }
</style>
