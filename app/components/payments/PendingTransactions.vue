<template>
  <div class="space-y-4 font-sans">
    <div class="flex items-center justify-between">
      <h4
        class="text-base font-semibold text-gray-900 dark:text-white font-subheading leading-tight"
      >
        Pending Payments
      </h4>
      <button
    ...
    <div v-if="loading && transactions.length === 0" class="animate-pulse">
      <div class="h-24 bg-gray-100 dark:bg-slate-800 rounded-lg" />
    </div>
    ...
    <div v-else class="space-y-3">
      ...
      >
        <div class="flex items-start justify-between mb-3">
          <div class="flex-1">
            <div class="flex items-center gap-2 mb-1">
              ...
              >
                {{ transaction.plan_id.toUpperCase() }} Plan
              </h5>
              <span
            ...
            <p class="text-xs text-gray-500 dark:text-gray-500 mt-1">
              Provider:
              <span class="capitalize">{{ transaction.provider }}</span>
            </p>
          </div>
        </div>
        ...
        >
          <p
            class="text-sm text-yellow-800 dark:text-yellow-300 flex items-start gap-2"
          >
            <svg
            ...
            <span
              >Payment initiated but not completed. If you already paid, click
              "Verify Payment" to complete activation.</span
            >
          </p>
        </div>
        ...
        <div class="flex gap-2">
          ...
          <button
            ...
            @click="resumePayment(transaction.checkout_url)"
          >
            Resume Payment
          </button>
        </div>
    ...
    >
      ...
      <button class="hover:text-red-100 text-sm" @click="errorMessage = ''">
        ×
      </button>
    </div>
  </div>
...
const fetchPendingTransactions = async () => {
  ...
  try {
    const response = await $fetch(
      `${config.public.apiBase}/api/payment/pending-transactions`,
      {
...
const verifyTransaction = async (transactionId) => {
  ...
  try {
    const response = await $fetch(
      `${config.public.apiBase}/api/payment/verify-manual`,
      {
        method: "POST",
        headers: { Authorization: `Bearer ${authStore.token}` },
    ...
    if (response.success) {
      successMessage.value =
        "Payment verified successfully! Refreshing subscription...";
