...
<template>
  <div
    class="flex h-[calc(100vh-80px)] bg-gray-100 dark:bg-slate-950 transition-colors duration-300 font-sans"
  >
    <!-- Sidebar: Tabs + List -->
    <div
      class="w-full md:w-[350px] lg:w-1/3 bg-white dark:bg-slate-900 flex flex-col shadow-lg md:shadow-none border-r border-gray-200 dark:border-slate-800"
    >
      <!-- Header -->
      <div
      ...
      >
        <h2
          class="text-xl font-bold text-gray-800 dark:text-white font-subheading leading-tight"
        >
          Agent Dashboard
        </h2>
        <div class="flex items-center space-x-2">
          ...
          <button
            class="p-2 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg transition"
            title="Refresh"
            @click="refreshAll"
          >
            <RefreshCcw
      ...
      <!-- Tabs -->
      <div
        class="flex border-b border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-900"
      >
        <button
          v-for="tab in tabs"
          :key="tab.key"
          class="flex-1 px-4 py-2 text-sm font-medium transition relative"
          :class="[
            activeTab === tab.key
        ...
        >
          {{ tab.label }}
          <span
            v-if="tab.badge && tab.badgeCount > 0"
      ...
      <div
        ...
        class="flex-1 overflow-y-auto p-2 bg-gray-50 dark:bg-slate-950"
      >
        <div
          v-if="handoverStore.pendingRequests.length === 0"
          class="text-center py-12 text-gray-500 dark:text-gray-400"
        >
          <Inbox
            class="w-12 h-12 mx-auto mb-2 text-gray-400 dark:text-gray-500"
          />
        ...
        <div
          v-for="request in handoverStore.pendingRequests"
          :key="request.id"
        ...
        >
          <div class="flex items-start justify-between mb-2">
            <div class="flex-1">
              <h3
                class="font-semibold text-gray-800 dark:text-white font-subheading leading-tight"
              >
                New Request
              </h3>
            ...
            <span
              class="px-2 py-1 text-xs font-medium text-orange-600 bg-orange-50 dark:bg-orange-900/20 dark:text-orange-300 rounded"
              >Pending</span
            >
          </div>

          <p class="text-sm text-gray-600 dark:text-gray-300 mb-3 line-clamp-2">
            {{ request.user_message || "Customer needs assistance" }}
          </p>

          <div class="flex space-x-2">
            <button
              :disabled="handoverStore.loading"
              class="flex-1 bg-green-500 hover:bg-green-600 text-white px-2 py-2 rounded text-sm font-medium transition disabled:opacity-50"
            ...
            <button
              :disabled="handoverStore.loading"
              class="flex-1 bg-gray-200 dark:bg-slate-700 hover:bg-gray-300 dark:hover:bg-slate-600 text-gray-700 dark:text-gray-200 px-2 py-2 rounded text-sm font-medium transition disabled:opacity-50"
      ...
      >
        <div
          v-if="handoverStore.activeConversations.length === 0"
          class="text-center py-12 text-gray-500 dark:text-gray-400"
        >
          <MessageCircle
            class="w-12 h-12 mx-auto mb-2 text-gray-400 dark:text-gray-500"
        ...
        <div
          v-for="conversation in handoverStore.activeConversations"
          :key="conversation.id"
        ...
        >
          ...
          <div class="flex-1">
            <h3
              class="text-sm font-semibold text-gray-900 dark:text-white font-subheading leading-tight"
            >
              Customer {{ conversation.session_id.slice(-6) }}
            </h3>
            <p class="text-xs text-gray-500 dark:text-gray-400">
              Active conversation
            </p>
    ...
    <!-- Chat Window -->
    <div
      class="flex-1 flex flex-col bg-white dark:bg-slate-900 md:bg-gray-50 dark:md:bg-slate-950 h-full"
    >
      <div
        v-if="selectedConversation"
        class="flex flex-col h-full bg-white dark:bg-slate-900 rounded-none md:rounded-tl-2xl shadow-xl overflow-hidden border-l border-gray-200 dark:border-slate-800"
      >
        <!-- Header -->
        <div
          class="p-4 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 shadow-sm flex items-center justify-between"
        >
          <div class="flex items-center space-x-3">
            <button
              class="sm:hidden text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"
            ...
            <div>
              <h2
                class="text-base font-semibold text-gray-800 dark:text-white font-subheading"
              >
                Customer {{ selectedConversation.session_id.slice(-6) }}
              </h2>
        ...
        <!-- Input -->
        <div
          class="p-3 md:p-4 bg-white dark:bg-slate-900 border-t border-gray-200 dark:border-slate-800 flex items-center space-x-3"
        >
          <input
            v-model="newMessage"
          ...
          <button
            ...
            class="p-4 bg-[#9E4CFF] text-white rounded-xl hover:bg-purple-700 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-md"
            @click="sendMessage"
          >
            <Send class="w-5 h-5" />
          </button>
      ...
      <div
        v-else
        class="flex-1 flex items-center justify-center text-gray-400 dark:text-gray-500 text-base"
      >
...
const tabs = computed(() => [
  {
    ...
    label: "Requests",
    badge: true,
    badgeCount: handoverStore.pendingCount,
  },
  {
    key: "active",
    label: "Active",
    badge: true,
    badgeCount: handoverStore.activeCount,
  },
...
const subscribeToConversation = (conversationId) => {
  ...
  channel.listen(".agent.message", (event) => {
    const conversation = handoverStore.activeConversations.find(
...
async function sendMessage() {
  ...
  newMessage.value = "";
  try {
    await handoverStore.sendAgentMessage(
...
function confirmEndHandover() {
  if (
    confirm(
      "Are you sure you want to end this conversation? The customer will be returned to the AI assistant.",
    )
  ) {
    endHandover();
  }
