<template>
  <!-- Fullscreen Editor -->
  <div class="fixed inset-0 bg-white z-50 flex flex-col font-sans">
    <!-- Header -->
    <div
    ...
    >
      <div class="flex items-center gap-4">
        ...
        <div>
          <h2 class="text-xl font-bold text-gray-900 font-subheading">
            {{ isEditing ? "Edit Template" : "Create Template" }}
          </h2>
          <p v-if="formData.name" class="text-sm text-gray-600">
            {{ formData.name }}
          </p>
        </div>
        <span
    ...
    <div class="flex-1 overflow-hidden">
      <div id="template-editor" class="w-full h-full" />
    </div>
...
const canPublish = computed(
  () => formData.value.name && formData.value.subject,
);
...
const initUnlayer = () => {
  ...
  setTimeout(() => {
    if (isEditing.value && props.template?.unlayer_design) {
      const design =
        typeof props.template.unlayer_design === "string"
...
const autoSave = async () => {
  ...
  try {
    ...
    templateId.value = await emailStore.autoSaveTemplate(
      formData.value,
      templateId.value,
    );
