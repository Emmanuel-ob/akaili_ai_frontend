...
<template>
  <div class="">
    <!-- Label -->
    <label
      class="block text-base font-semibold text-gray-900 dark:text-gray-200 font-subheading"
    ...
    <div class="flex items-center mt-2 space-x-3">
      <!-- Color preview -->
      <div
        class="w-12 h-12 rounded-lg border border-gray-300 dark:border-slate-600 shadow cursor-pointer transition-colors"
      ...
      <input
        ...
        type="color"
        :value="colorWithHash"
        class="w-0 h-0 opacity-0 absolute font-sans text-sm"
      ...
      <div class="flex-1">
        <div class="relative">
          <span
            class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 dark:text-gray-400 font-mono"
            >#</span
          >
          <input
            :value="colorValueDisplay"
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 font-sans">
          Enter 6-digit hex color code (without #)
        </p>
      </div>
    </div>
  </div>
...
const colorWithHash = computed(() => {
  const value = props.modelValue || "#6366f1";
